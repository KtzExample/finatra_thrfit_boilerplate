...
class ExampleController @Inject() (fakeDatabase: FakeDatabase) extends Controller with TUserService.BaseServiceIface {
  override val getAllUserInfo: Service[GetAllUserInfo.Args, GetAllUserInfo.SuccessType] = handle(GetAllUserInfo) { _ =>
    Future.value(fakeDatabase.userInfo.toList.map(_._2))
  }

  override val getUserInfoById: Service[GetUserInfoById.Args, GetUserInfoById.SuccessType] = handle(GetUserInfoById) { args =>
    Future.value(fakeDatabase.userInfo.get(args.userId).toSeq)
  }

  override val setUserInfoById: Service[SetUserInfoById.Args, SetUserInfoById.SuccessType] = handle(SetUserInfoById) { args =>
    Future.value(fakeDatabase.set(args.userInfoToSet).toSeq)
  }

  override val getCarInfoById: Service[GetCarInfoById.Args, GetCarInfoById.SuccessType] = handle(GetCarInfoById) { args =>
